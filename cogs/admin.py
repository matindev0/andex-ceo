...
@app_commands.command(
    name="setup",
    description="(Owner) دووری Aria دابنێ — چەناڵەکانی Command، Chat، Ideas، و Logs دیاری بکە",
)
...
async def authorize(
    ...
    await interaction.response.send_message(
        f"✅ **{user.display_name}** ئەستێنرا — ئێستا دەتوانێت بە Aria قسە بکات.",
        ephemeral=True,
    )
...
async def status(self, interaction: discord.Interaction) -> None:
    ...
    embed.add_field(
        name="📡 چەناڵەکان",
        value="\n".join(f"`{role}`: <#{ch_id}>" for role, ch_id in channels.items()) or "هێشتا سازنەکراوە",
        inline=False,
    )
    embed.set_footer(text="Everything is under control AnDex")
    await interaction.response.send_message(embed=embed, ephemeral=True)
...
@app_commands.describe(channel="چەناڵی مێژووی پاکبووە")
@owner_only()
